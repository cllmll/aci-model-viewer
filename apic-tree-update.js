...
function updateLinks(source, treeData)
{
    ...
    // if the child node (d) is a target of the parent node (ie. parentnode contains child node) 
    // markers set on :
    // 1. Parent containing child
    // 2. Rs (Reln From)
    // 3. Rt (Reln To)    
    // need code to rotate and position marker for cruved lines
    // .attr("marker-start", 
    //                     function(d)
    //                     {
    ...
    //                         if(pDn1 === d.parent.data.dn) { return "url(#childtarget)"; }     
    //                         // is d.parent (pDn2) is contained child of d ?
    //                         if(pDn2 === d.data.dn) { return "url(#parenttarget)"; }    
    //                         // relationship
    //                         return "";
    //                     });
    ...
    d3.selectAll('g.node')  // get all the nodes
        .each(function(d) 
        {
            ...
            d3.select(this).select('text#textinfo').select('tspan#nodelabel')
                ...
                .attr("dx",
                    function(d) {
                        return (d.children && (d.children.length % 2 != 0)) ? 0 : 0;}) ; //13,13
